...
func (d *RepoDbClipboard) Create(ctx context.Context, clip model.Clipboard) error {
	tx, err := repo.Begin(ctx, d.db) // เพื่อที่จะใช้งาน Rollback
	if err != nil {
		return err
	}
...
func (d *RepoDbClipboard) CheckUserID(ctx context.Context, id string, userID string) (bool, error) {
	rows, err := d.db.QueryContext(ctx, "select * from CLIPBOARDS")
	if err != nil {
		return false, fmt.Errorf("query select err: %w", err)
	}
	...
	for rows.Next() {
		err := rows.Scan(&clipboard.Id, &clipboard.UserId, &clipboard.Text)
		if err != nil {
			return false, fmt.Errorf("scan err: %w", err)
		}
	}
	...
	return false, fmt.Errorf("no such clipboard for userID: %s", userID)
}

func (d *RepoDbClipboard) GetUserClipboard(ctx context.Context, id, userID string) (model.Clipboard, error) {
	//เช็ค user_id ตรงนี้ดีไหม หรือไม่ต้องเช็คเพราะ qury ด้วย user_id อยู่แล้ว  หรือควรเช็คข้างล่าง
	clip, err := d.GetById(ctx, id)
	if err != nil {
		return model.Clipboard{}, fmt.Errorf("get-by-id for check userID err: %w", err)
	}
	...
	a, err := d.CheckUserID(ctx, id, userID)
	if err != nil || !a {
...
// ที่ผิดตอนแรก
func (d *RepoDbClipboard) DeleteUserClipboard_version_OLD(ctx context.Context, id, userId string) error {
	tx, err := repo.Begin(ctx, d.db)
	if err != nil {
		return err
	}
